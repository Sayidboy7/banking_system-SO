...
@app.route('/<string:bank_name>/register/bank-account', methods=['POST', 'GET'])
@login_required
def create_bank_account(bank_name):
    ...
    if request.method == 'POST':
        ...
        if not login_name or not comment or not code:
            flash('Please complete the form!', category='warning')
            return redirect(url_for('create_bank_account', bank_name=bank.name))
        ...
        if len(code) < 4:
            ...
            return redirect(url_for('create_bank_account', bank_name=bank_name))
        ...
        bank_account = BankAccount(
            ...
            balance=deposit if deposit and deposit <= 5 else 0
        )
    ...
    return render_template('register_bank_account.html', len_bank_accounts=len_bank_accounts, bank=bank)
